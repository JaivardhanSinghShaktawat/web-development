...
If the domain name of the website is of .com than root server will return the IP address of the TLD Server containing .com domain IP addresses to the recursive server.Root server returns the IP address of a specific TLD server containing information about the specific domain name. (.com , .in , .edu)

**TLD Server**
It stands for Top Level Domain.
it is a collection of different servers where each server stores some information of related to a specific domain name (.com , .in , .edu etc.)
...
it stores the IP address of the server.
...
**Step 5 :** Root Server --> Recursive Server (root returns the address of a specific TLD server according based on the domain name of the url)
