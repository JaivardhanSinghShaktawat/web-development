...
When the request goes to the server than IP address , MAC address , request and a PORT number is attached.When the response comes to the client than the response will contain the IP address , MAC address and PORT number which will help OS to decide ki kis running application ko data send krna h.
