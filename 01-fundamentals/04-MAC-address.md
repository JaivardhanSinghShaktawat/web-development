...
Now , suppose a new user z connects with internet and ISP gives A as IP address to z.Now , after 5 minutes the server y with IP address B will give the result to z which has IP address as A.
...
MAC address is used to send the data to the correct user/device/machine.
...
It has 6 groups and each group has 2 hexadecimal digits.Each group size is 8 bits and
each digit size is 4 bits.
...
1. To send the data to the correct device.
2. To send/share the data in different ways.
3. To identify a deivce uniquely.
