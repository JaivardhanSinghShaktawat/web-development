...
ISP (Interet Service Provider) gives a temporary IP address to our device each time when we connect to the internet.
...
ISP can give the same IP address to different devices but not at the same
time.ISP changes rapidly when we connect to the internet.
